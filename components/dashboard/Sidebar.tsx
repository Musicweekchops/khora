...
const TEACHER_NAV = [
  ...
  { name: "Mi Landing", href: "/dashboard/landing", icon: (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
...
export default function Sidebar({ user }: SidebarProps) {
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <aside className="hidden lg:flex flex-col h-screen w-[240px] fixed left-0 top-0 z-50 border-r bg-white border-neutral-200/80 text-neutral-900">
        {/* Logo */}
        <div className="h-16 flex items-center gap-3 px-5 border-b border-neutral-100">
          {academyBrand ? (
            ...
            <>
              ...
              <div>
                <h1 className="text-sm font-semibold tracking-tight leading-none text-neutral-900">Khora</h1>
                <p className="text-[10px] font-medium mt-0.5 text-neutral-400">Gestión de clases</p>
              </div>
            </>
          )}
        </div>
        ...
        <div className="p-3 border-t border-neutral-100 bg-neutral-50/50">
          <div className="flex items-center gap-3 px-3 py-2 mb-1">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[13px] font-medium truncate leading-tight text-neutral-900">{user.name}</p>
              {user.role === 'TEACHER' ? (
                ...
                <p className="text-[11px] capitalize mt-0.5 text-neutral-400">{user.role.toLowerCase()}</p>
              )}
            </div>
          </div>
          ...
          <button
            ...
            className="w-full flex items-center gap-2 px-3 py-2 rounded-lg transition-colors text-[13px] font-medium mb-1 text-neutral-500 hover:text-violet-600 hover:bg-violet-50"
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
              <circle cx="12" cy="12" r="10" />
          ...
          <button
            onClick={() => signOut()}
            className="w-full flex items-center gap-2 px-3 py-2 rounded-lg transition-colors text-[13px] font-medium text-neutral-500 hover:text-red-650 hover:bg-red-50"
          >
            {Icons.logout}
      ...
      <nav 
        className="lg:hidden fixed left-4 right-4 z-50 backdrop-blur-md border shadow-[0_8px_32px_rgba(0,0,0,0.08)] py-3 px-2 rounded-2xl animate-in slide-in-from-bottom duration-300 bg-white/95 border-neutral-200/60"
        style={{ bottom: 'calc(env(safe-area-inset-bottom, 16px) + 12px)' }}
      >
        <div className="flex items-center justify-around">
          {navItems.filter(item => !(isStudent && item.href === "/dashboard/ajustes")).slice(0, 3).map(item => {
            const isActive = pathname === item.href || (item.href !== "/dashboard" && pathname?.startsWith(item.href))
            return (
          ...
          <button
            onClick={() => window.dispatchEvent(new CustomEvent("khora-trigger-onboarding"))}
            className="flex-1 flex flex-col items-center justify-center gap-1 text-neutral-400 hover:text-violet-600"
          >
