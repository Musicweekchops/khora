...
export default function FinancialDashboard() {
  ...
  useEffect(() => {
    if (profile?.teacherProfileId) {
      fetchSummary(profile.teacherProfileId)
    }
  }, [profile?.teacherProfileId])

  const fetchSummary = async (teacherId: string) => {
    try {
      // 1. Obtener todos los pagos vinculados a alumnos de este profesor
      const { data: allPayments, error: paymentsError } = await supabase
        .from('Payment')
        .select(`
          *,
          student:StudentProfile!inner(
            id,
      ...
      // 2. Obtener todos los alumnos activos del profesor
      ...
      const studentsWithDebt = activeStudents?.filter(s => 
        (s.payments as any[])?.some((p: any) => p.status === "PENDING" && p.dueDate && new Date(p.dueDate) < now)
      ) || []
      ...
      setSummary({
        ...
        studentsWithDebt: studentsWithDebt.map(s => ({
          id: s.id,
          name: (s.user as any)?.name || "Sin nombre",
          overduePayments: (s.payments as any[])
